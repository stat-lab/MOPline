...
#@tools = ('BreakDancer', 'inGAP', 'Lumpy', 'Delly', 'CNVnator', 'SoftSV', 'Mobster.MEI', 'MELT.MEI', 'Basil', 'forestSV', 'Wham', 'Manta', 'MATCHCLIP', 'GRIDSS');

#@tools = ('inGAP', 'CNVnator', 'MELT.MEI', 'Manta', 'Wham', 'MATCHCLIP');
my @tools_2 = ('inGAP', 'CNVnator', 'MELT.MEI', 'Manta', 'SoftSV', 'Wham');
...
foreach my $id (keys %var_file){
    ...
    while (my $line = <FILE>){
        ...
        ${${${${$tool_sv{$id}}{$type}}{$size}}{$chr}}{$pos} = $line;
        if (($len <= 1100) and ($len > 1000)){
            ${${${${$tool_sv{$id}}{$type}}{'S'}}{$chr}}{$pos} = $line;
        }
        if (($len >= 900) and ($len <= 1000)){
            ${${${${$tool_sv{$id}}{$type}}{'M'}}{$chr}}{$pos} = $line;
        }
        if (($len >= 90000) and ($len <= 100000)){
            ${${${${$tool_sv{$id}}{$type}}{'L'}}{$chr}}{$pos} = $line;
        }
        if (($len > 100000) and ($len <= 110000)){
            ${${${${$tool_sv{$id}}{$type}}{'M'}}{$chr}}{$pos} = $line;
        }
    }
    close (FILE);
    ...
    foreach my $type (keys %{$tool_sv{$id}}){
        foreach my $size (keys %{${$tool_sv{$id}}{$type}}){
            foreach my $chr (keys %{${${$tool_sv{$id}}{$type}}{$size}}){
                my $pre_pos = 0;
                my $pre_len = 0;
                my $pre_read = 0;
                    ...
                    if ($pre_pos > 0){
                        if ($type eq 'INS'){
                            if ($pos - $pre_pos <= $ins_sd2){
                                if ($pre_read >= $read){
                                    delete ${${${${$tool_sv{$id}}{$type}}{$size}}{$chr}}{$pos};
                                    next;
                                }
                                else{
                                    delete ${${${${$tool_sv{$id}}{$type}}{$size}}{$chr}}{$pre_pos};
                                }
                            }
                        }
                        else{
                            ...
                            if ($pos < $pre_end){
                                ...
                                if ($type ne 'DEL'){
                                    ...
                                    if ($pre_read >= $read * $min_read_fold){
                                        ...
                                        my $pre_line = ${${${${$tool_sv{$id}}{$type}}{$size}}{$chr}}{$pre_pos};
                                        ...
                                        ${${${${$tool_sv{$id}}{$type}}{$size}}{$chr}}{$pre_pos} = $pre_line;
                                        delete ${${${${$tool_sv{$id}}{$type}}{$size}}{$chr}}{$pos};
                                        $pre_len = $new_len;
                                        $pre_read = $new_read;
                                        next;
...
=cut
                    ...
                    my $subtype = $line[2];
                    if ((!exists ${${${$call{$type}}{$size}}{$chr}}{$pos}) or ($type eq 'INS')){
                        push @{${${${$call{$type}}{$size}}{$chr}}{$pos}}, "$id=$pos=$len=$reads" if ($type ne 'INS');
                        push @{${${${$call{$type}}{$size}}{$chr}}{$pos}}, "$id=$pos=$len=$reads=$subtype" if ($type eq 'INS');
                    }
                    else{
                        my $sumlen = 0;
                        foreach (@{${${${$call{$type}}{$size}}{$chr}}{$pos}}){
                            my ($id2, $pos2, $len2) = split (/=/, $_);
                            $sumlen += $len2;
                        }
                        my $avelen = int ($sumlen / @{${${${$call{$type}}{$size}}{$chr}}{$pos}} + 0.5);
                        if (($len / $avelen > 2) or ($len / $avelen < 0.5)){
                            if (@{${${${$call{$type}}{$size}}{$chr}}{$pos}} == 1){
                                if ($avelen < $len){
                                    delete ${${${$call{$type}}{$size}}{$chr}}{$pos};
                                    push @{${${${$call{$type}}{$size}}{$chr}}{$pos}}, "$id=$pos=$len=$reads";
                                }
                            }
                        }
                        else{
...
foreach my $type (keys %call){
    foreach my $size (keys %{$call{$type}}){
        foreach my $chr (keys %{${$call{$type}}{$size}}){
            my $pre_pos = 0;
            my $pre_len = 0;
            foreach my $pos (sort {$a <=> $b} keys %{${${$call{$type}}{$size}}{$chr}}){
                my $sum_len = 0;
                my @len;
                foreach (@{${${${$call{$type}}{$size}}{$chr}}{$pos}}){
                ...
                if ($pre_pos > 0){
                    if ($type eq 'INS'){
                        if ($pos - $pre_pos <= $var_sd){
                            my $pre_num = scalar @{${${${$call{$type}}{$size}}{$chr}}{$pre_pos}};
                            my $num = scalar @{${${${$call{$type}}{$size}}{$chr}}{$pos}};
                            my $new_pos = int (($pre_pos * $pre_num + $pos * $num) / ($pre_num + $num) + 0.5);
                            my @new_info = (@{${${${$call{$type}}{$size}}{$chr}}{$pre_pos}}, @{${${${$call{$type}}{$size}}{$chr}}{$pos}});
                            delete ${${${$call{$type}}{$size}}{$chr}}{$pre_pos};
                            delete ${${${$call{$type}}{$size}}{$chr}}{$pos};
                            push @{${${${$call{$type}}{$size}}{$chr}}{$new_pos}}, @new_info;
                            $pre_pos = $new_pos;
                            next;
                        }
                    }
...
foreach my $type (keys %call){
    foreach my $size (keys %{$call{$type}}){
        foreach my $chr (keys %{${$call{$type}}{$size}}){
            foreach my $pos (sort {$a <=> $b} keys %{${${$call{$type}}{$size}}{$chr}}){
                if (@{${${${$call{$type}}{$size}}{$chr}}{$pos}} == 1){
                    my ($tool1, $tpos, $tlen, $tread, $subtype) = split (/=/, ${${${${$call{$type}}{$size}}{$chr}}{$pos}}[0]);
                    ${${${$call_merge{$type}}{$size}}{$chr}}{$pos} = "$tool1:$tread=$tpos=$tlen=$tread" if ($type ne 'INS');
                    ${${${$call_merge{$type}}{$size}}{$chr}}{$pos} = "$tool1:$tread=$tpos=$tlen=$tread=$subtype" if ($type eq 'INS');
                }
                else{
                    my %tool_info;
                    my %bp_rank;
                    ...
                    foreach (@{${${${$call{$type}}{$size}}{$chr}}{$pos}}){
                        ...
                        my $preci1 = 1;
                        $preci1 = ${${${$tool_preci{$type}}{$size}}{$tool2}}{$read1} if (exists ${${${$tool_preci{$type}}{$size}}{$tool2}}{$read1});
...
foreach my $type (keys %call_merge){
    foreach my $size (keys %{$call_merge{$type}}){
            ...
            while (1){
                    ...
                    if ($pre_pos > 0){
                        if ($type eq 'INS'){
                            ...
                            foreach my $pos1 (@merge){
                                my ($toolset1, $tpos1, $len1, $read1, $subtype) = split (/=/, ${${${$call_merge{$type}}{$size}}{$chr}}{$pos1});
                                ...
                                if ($setnum == 1){
                                    ...
                                    foreach (@{$toolset{$toolnum}}){
                                        ...
                                        foreach my $tool1 (@toolset){
                                            ...
                                            $tool2 =~ s/\.NUMT$// if ($tool2 =~ /\.NUMT$/);
                                            my $preci1 = 1;
                                            $preci1 = ${${${$tool_preci{$type}}{$size}}{$tool2}}{$read2} if (exists ${${${$tool_preci{$type}}{$size}}{$tool2}}{$read2});
                            ...
                            if ($select_pos > 0){
                                $delete ++ if (@merge > 1);
                                ${${${$call_merge{$type}}{$size}}{$chr}}{$select_pos} = "$tool_set=$select_pos=0=$select_read=$subtype_set";
                            }
                            @merge = ();
                            push @merge, $pos;
                        }
                            ...
                            if ($pos < $pre_end){
                                my $overlap = $pre_end - $pos + 1;
                                $overlap = $len if ($end < $pre_end);
                                if (($overlap >= $len * $min_overlap_ratio4) and ($overlap >= $pre_len * $min_overlap_ratio4)){
                                    push @merge, $pos;
                                    $match_count ++;
                                    next;
                                }
                            ...
                            foreach my $pos1 (@merge){
                                ...
                                my ($toolset1, $tpos1, $len1, $read1) = split (/=/, ${${${$call_merge{$type}}{$size}}{$chr}}{$pos1});
                                my @toolset = split (/,/, $toolset1);
                                my $new_toolset = '';
                                foreach (@toolset){
                                    if ($_ !~ /:\d+:/){
                ...
                if (@merge > 0){
                    ...
                    if ($select_pos > 0){
                        $delete ++ if (@merge > 1);
                        ${${${$call_merge{$type}}{$size}}{$chr}}{$select_pos} = "$tool_set=$select_pos=$select_len=$select_read" if ($type ne 'INS');
                        ${${${$call_merge{$type}}{$size}}{$chr}}{$select_pos} = "$tool_set=$select_pos=$select_len=$select_read=$subtype_set" if ($type eq 'INS');
                    }
                    @merge = ();
                }
                last if ($delete == 0);
...
foreach my $type (keys %call_merge){
    ...
    if ((exists ${$call_merge{$type}}{'S'}) and (exists ${$call_merge{$type}}{'M'})){
        foreach my $chr (keys %{${$call_merge{$type}}{'S'}}){
            ...
            foreach my $pos1 (keys %{${${$call_merge{$type}}{'S'}}{$chr}}){
                my ($toolsetS, $posS, $lenS, $readS, $subtypesetS) = split (/=/, ${${${$call_merge{$type}}{'S'}}{$chr}}{$pos1});
                my $end1 = $pos1 + $lenS - 1;
                foreach my $pos2 (keys %{${${$call_merge{$type}}{'M'}}{$chr}}){
                    last if ($pos2 > $end1);
                    my ($toolsetM, $posM, $lenM, $readM, $subtypesetM) = split (/=/, ${${${$call_merge{$type}}{'M'}}{$chr}}{$pos2});
                    my $end2 = $pos2 + $lenM - 1;
                    next if ($end2 < $pos1);
                    my $flag = 0;
                    ...
                    if (($pos1 >= $pos2) and ($pos1 <= $end2)){
                        my $overlap = $end2 - $pos1 + 1;
                        if ($end1 <= $end2){
                            if ($lenS >= $lenM * $min_overlap_ratio4){
                                $flag = 1;
                            }
                        }
                        elsif (($overlap >= $lenS * $min_overlap_ratio4) or ($overlap >= $lenM * $min_overlap_ratio4)){
                            if (($overlap >= $lenS * $min_overlap_ratio4) and ($overlap >= $lenM * $min_overlap_ratio4)){
                                $flag = 1;
                            }
                            elsif ((($lenS >= $lenM) and ($lenS < $lenM * 5)) or (($lenS < $lenM) and ($lenM < $lenS * 5))){
                                $flag = 1;
                            }
                        }
                    }
                        ...
                        if ($end2 <= $end1){
                            if ($lenM >= $lenS * $min_overlap_ratio4){
                                $flag = 1;
                            }
                        }
                        elsif (($overlap >= $lenS * $min_overlap_ratio4) or ($overlap >= $lenM * $min_overlap_ratio4)){
                    ...
                    if ($flag == 1){
                        my @toolsetS = split (/,/, $toolsetS);
                        my @toolsetM = split (/,/, $toolsetM);
                        if (@toolsetS > @toolsetM){
                            ${${${$duplicate{$type}}{'M'}}{$chr}}{$pos2} = 1;
                        }
                        elsif (@toolsetS < @toolsetM){
                            ${${${$duplicate{$type}}{'S'}}{$chr}}{$pos1} = 1;
                        }
                        else{
                            if ($lenS > 1000){
                                ${${${$duplicate{$type}}{'S'}}{$chr}}{$pos1} = 1;
    ...
    if ((exists ${$call_merge{$type}}{'M'}) and (exists ${$call_merge{$type}}{'L'})){
        foreach my $chr (keys %{${$call_merge{$type}}{'M'}}){
            next if (!exists ${${$call_merge{$type}}{'L'}}{$chr});
            foreach my $pos1 (keys %{${${$call_merge{$type}}{'M'}}{$chr}}){
                my ($toolsetM, $posM, $lenM, $readM, $subtypesetM) = split (/=/, ${${${$call_merge{$type}}{'M'}}{$chr}}{$pos1});
                my $end1 = $pos1 + $lenM - 1;
                foreach my $pos2 (keys %{${${$call_merge{$type}}{'L'}}{$chr}}){
                    last if ($pos2 > $end1);
                    my ($toolsetL, $posL, $lenL, $readL, $subtypesetL) = split (/=/, ${${${$call_merge{$type}}{'L'}}{$chr}}{$pos2});
                    my $end2 = $pos2 + $lenL - 1;
                    next if ($end2 < $pos1);
                    my $flag = 0;
                    if ((abs ($pos1 - $pos2) <= $var_sd) and (abs ($end1 - $end2) <= $var_sd)){
                    ...
                    if (($pos1 >= $pos2) and ($pos1 <= $end2)){
                        my $overlap = $end2 - $pos1 + 1;
                        if ($end1 <= $end2){
                            if ($lenM >= $lenL * $min_overlap_ratio4){
                                $flag = 1;
                            }
                        }
                        elsif (($overlap >= $lenM * $min_overlap_ratio4) or ($overlap >= $lenL * $min_overlap_ratio4)){
                            if (($overlap >= $lenM * $min_overlap_ratio4) and ($overlap >= $lenL * $min_overlap_ratio4)){
                                $flag = 1;
                            }
                            elsif ((($lenM >= $lenL) and ($lenM < $lenL * 5)) or (($lenM < $lenL) and ($lenL < $lenM * 5))){
                                $flag = 1;
                            }
                        }
                    }
                        ...
                        my $overlap = $end1 - $pos2 + 1;
                        if ($end2 <= $end1){
                            if ($lenL >= $lenM * $min_overlap_ratio4){
                                $flag = 1;
                            }
                        }
                        elsif (($overlap >= $lenM * $min_overlap_ratio4) or ($overlap >= $lenL * $min_overlap_ratio4)){
                    ...
                    if ($flag == 1){
                        my @toolsetM = split (/,/, $toolsetM);
                        my @toolsetL = split (/,/, $toolsetL);
                        if (@toolsetM > @toolsetL){
                            ${${${$duplicate{$type}}{'L'}}{$chr}}{$pos2} = 1;
                        }
                        elsif (@toolsetM < @toolsetL){
                            ${${${$duplicate{$type}}{'M'}}{$chr}}{$pos1} = 1;
                        }
                        else{
                            if ($lenM > 100000){
                                ${${${$duplicate{$type}}{'M'}}{$chr}}{$pos1} = 1;
