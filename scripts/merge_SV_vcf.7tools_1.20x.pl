...
=head1 SYNOPSIS
  ...
  merge_SV_vcf.7tools_1.20x.pl -s <sample-list-file or a sample name> -rl <read_length> (-nh 1 if sample is a non-human species)
...
foreach my $ID (@ID_list){
    ...
    system ("$script_dir/merge_SV_calls_multi_tools.pl -st INS -rl $read_length -s $ID -d $ID -t $ins_tools -nh $non_human > $ID/$merge_dir/Merge.INS.5tools.simple.vcf");
    ...
    my $ins_set = "Manta:3=inGAP:$inGAP_ins_mr1 Wham:3=inGAP:$inGAP_ins_mr1 Manta:3=Wham:3 INSurVeyor:3=Manta:3 INSurVeyor:3=Wham:3 INSurVeyor:3=inGAP:$inGAP_ins_mr1 inGAP:$inGAP_ins_mr2 Manta:18 MELT:3 INSurVeyor:12";
