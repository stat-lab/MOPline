...
=head1 SYNOPSIS
  ...
  add_GT_DPR_vcf_single.pl -v <vcf file path> -sn <sample_name> -sd <sample_directory> -ts <tool_set> (-ri <reference_index> -gap <gap_bed> -nh 1 if sample is a non-human species)
  (Add genotype and alignment information (GT/DR/DS/SR tags) to the genotype field of a single MOP-merged vcf file with cov files that were generated with add_GT_DPR_vcf_thread.pl)
