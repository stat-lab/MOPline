...
=head1 SYNOPSIS
  ...
  merge_SV_vcf.4tools.pl -s <sample-list-file or a sample name> -rl <read_length> (-nh 1 if sample is a non-human species)
  output vcf file name: ${sample_name}.Merge.ALL.vcf
  Tools used: inGAP, CNVnator, Manta, Wham
  ...
  Options:
   ...
   --dir or -d <STR>            output directory name, which contains output vcf files [default: Merge_4tools]
...
my @tools = ('inGAP', 'CNVnator', 'Manta', 'Wham');
...
foreach my $ID (@ID_list){
    ...
    my $ins_tools = 'inGAP Manta Wham';
    
    #merge INS call sets from multiple tools with non-redundancy
    system ("$script_dir/merge_SV_calls_multi_tools.pl -st INS -rl $read_length -s $ID -d $IDdir -t $ins_tools -nh $non_human > $IDdir/$merge_dir/Merge.INS.3tools.simple.vcf");
    ...
    my $ins_set = "Manta:3=inGAP:$inGAP_ins_mr1 Wham:3=inGAP:$inGAP_ins_mr1 Manta:3=Wham:3 inGAP:$inGAP_ins_mr2";
    
    #select high quality overlap calls or high quality single calls from the merged INS calls
    system ("$script_dir/merge_SV_calls_multi_tools_filter.pl -t INS -ins $ins_set -v $IDdir/$merge_dir/Merge.INS.3tools.simple.vcf > $IDdir/$merge_dir/MOP.Merge.INS.3tools.vcf");
    
    my $del_tools = 'CNVnator inGAP Manta Wham';
    
    #merge DEL call sets from multiple tools with non-redundancy
    system ("$script_dir/merge_SV_calls_multi_tools_eachSize.pl -st DEL -rl $read_length -s $ID -d $IDdir -t $del_tools -nh $non_human > $IDdir/$merge_dir/Merge.DEL.4tools.simple.vcf");
    
    my $del_set_S = 'Manta:3=inGAP:7 Wham:5=Manta:3 Wham:5=inGAP:7';
    my $del_set_M = 'CNVnator:3=# Manta:3=Wham:3 Manta:3=inGAP:5 Wham:3=inGAP:5 CNVnator:7'; # inGAP:8 Wham:12';
    my $del_set_L = "CNVnator:3=# Manta:3=Wham:3 Manta:3=inGAP:5 Wham:3=inGAP:5 CNVnator:8 inGAP:$inGAP_del_mr1";
    
    #select high quality overlap calls or high quality single calls from the merged DEL calls
    system ("$script_dir/merge_SV_calls_multi_tools_filter.pl -t DEL -del_s $del_set_S -del_m $del_set_M -del_l $del_set_L -v $IDdir/$merge_dir/Merge.DEL.4tools.simple.vcf > $IDdir/$merge_dir/MOP.Merge.DEL.4tools.vcf");
    
    my $dup_tools = 'CNVnator inGAP Manta Wham';
    
    #merge DUP call sets from multiple tools with non-redundancy  
    system ("$script_dir/merge_SV_calls_multi_tools_eachSize.pl -st DUP -rl $read_length -s $ID -d $IDdir -t $dup_tools -nh $non_human > $IDdir/$merge_dir/Merge.DUP.4tools.simple.vcf");
    
    my $dup_set_S = 'Wham:4=Manta:4';
    my $dup_set_M = 'CNVnator:3=# inGAP:3=Manta:3 CNVnator:6';
    my $dup_set_L = 'CNVnator:2=Manta:3 Manta:3=Wham:3 CNVnator:5 inGAP:4';
    
    #select high quality overlap calls or high quality single calls from the merged DUP calls
    system ("$script_dir/merge_SV_calls_multi_tools_filter.pl -t DUP -dup_s $dup_set_S -dup_m $dup_set_M -dup_l $dup_set_L -v $IDdir/$merge_dir/Merge.DUP.4tools.simple.vcf > $IDdir/$merge_dir/MOP.Merge.DUP.4tools.vcf");
        
    my $inv_tools = 'inGAP Manta Wham';
    
    #merge INV call sets from multiple tools with non-redundancy
    system ("$script_dir/merge_SV_calls_multi_tools_eachSize.pl -st INV -rl $read_length -s $ID -d $IDdir -t $inv_tools -nh $non_human > $IDdir/$merge_dir/Merge.INV.3tools.simple.vcf");
    ...
    my $inv_set_L = 'inGAP:5=Manta:3 Wham:5=inGAP:5 Manta:3=Wham:5';
    
    #select high quality overlap INV calls
    system ("$script_dir/merge_SV_calls_multi_tools_filter.pl -t INV -inv_s $inv_set_S -inv_m $inv_set_M -inv_l $inv_set_L -v $IDdir/$merge_dir/Merge.INV.3tools.simple.vcf > $IDdir/$merge_dir/MOP.Merge.INV.3tools.vcf");
    ...
    system ("$script_dir/merge_SV_calls_ALLtype.pl -rl $read_length -v $IDdir/$merge_dir/MOP.Merge.DEL.4tools.vcf $IDdir/$merge_dir/MOP.Merge.INS.3tools.vcf $IDdir/$merge_dir/MOP.Merge.DUP.4tools.vcf $IDdir/$merge_dir/MOP.Merge.INV.3tools.vcf > $out_vcf");
