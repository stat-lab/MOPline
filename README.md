...
Other than MELT, which has license restrictions, the above 10 SV detection algorithms can be run using a singularity image container built using the Definition file (mopline.def) included in the package.  
...
```
To run the run_single_singularity.pl script for a single sample, install this MOPline package on the approprimate location and run as follows:  
```
